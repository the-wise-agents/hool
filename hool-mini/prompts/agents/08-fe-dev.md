...
1. After writing/updating tests, run them: execute the actual test command (e.g., `npm test`, `npx vitest run`, `npx jest`)
2. **Paste the actual terminal output** showing pass/fail counts in your work log
3. After implementation, run the FULL test suite (not just your tests): paste terminal output
4. If tests fail: fix the code, re-run, paste the new output
5. Your task is NOT complete until you have pasted passing test output

**Example of required evidence:**
```
$ npm test
 ✓ LoginForm renders correctly (12ms)
 ✓ LoginForm validates email (8ms)
 ✓ LoginForm submits credentials (15ms)
 Tests: 3 passed, 3 total
```

**NOT acceptable:** "I wrote tests for LoginForm and they pass." (no terminal output = violation)
